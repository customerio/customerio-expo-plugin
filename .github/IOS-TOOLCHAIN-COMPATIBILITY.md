...
`Xcode 27 preview compatibility` generates an app with the latest published Expo SDK, builds APN and FCM configurations on the floating Xcode 27 preview runner, installs each generated simulator product, launches it, and verifies that the expected executable remains alive for the configured survival window. It runs nightly at 07:17 UTC, on manual dispatch, and when its own workflow definition changes. Ordinary source pull requests and pushes do not trigger it; a pull request changing the workflow intentionally opts into the preview jobs.
