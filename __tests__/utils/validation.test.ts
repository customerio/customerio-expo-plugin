...
describe('validateNativeSDKConfig - Ensures SDK configuration meets requirements before initialization', () => {
  ...
  // Test strict mode, non-strict mode, and undefined (default production behavior)
  const testModes = [
    { name: 'strict mode', strictMode: 'true', expectThrow: true },
    { name: 'non-strict mode', strictMode: 'false', expectThrow: false },
    { name: 'undefined mode (default)', strictMode: undefined, expectThrow: false }
  ] as const;

  // Dual-mode testing for key validation scenarios
  describe('dual-mode validation tests', () => {
    testModes.forEach(({ name, strictMode, expectThrow }) => {
      describe(name, () => {
        let originalStrictMode: string | undefined;
        let consoleSpy: jest.SpyInstance;

        beforeEach(() => {
          originalStrictMode = process.env.CUSTOMERIO_STRICT_MODE;
          if (strictMode === undefined) {
            delete process.env.CUSTOMERIO_STRICT_MODE;
          } else {
            process.env.CUSTOMERIO_STRICT_MODE = strictMode;
          }
        ...
        afterEach(() => {
          if (originalStrictMode === undefined) {
            ...
            process.env.CUSTOMERIO_STRICT_MODE = originalStrictMode;
          }
          consoleSpy.mockRestore();
        });

        const testValidation = (config: any, shouldFail: boolean, expectedMessage?: string) => {
          if (expectThrow && shouldFail) {
            expect(() => validateNativeSDKConfig(config)).toThrow(expectedMessage);
          } else if (!expectThrow && shouldFail) {
        ...
        test('should handle cdpApiKey missing', () => {
          const config = {} as any;
          testValidation(config, true, 'NativeSDKConfig: cdpApiKey is required, received: undefined');
        });

        test('should handle cdpApiKey empty string', () => {
          const config = { cdpApiKey: '' };
          testValidation(config, true, 'NativeSDKConfig: cdpApiKey must be a non-empty string, received: ""');
        });
      });
    });
  });

  // Original tests with strict mode enabled
  describe('original validation tests (strict mode)', () => {
    let originalStrictMode: string | undefined;
    ...
    describe('cdpApiKey validation (remaining cases)', () => {
