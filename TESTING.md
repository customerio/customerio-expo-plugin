...
| `root-tests` | `__tests__/` (excluding `__tests__/scenarios/`) | Legacy mock-heavy unit tests + snapshot tests. Tests under `__tests__/{ios,android}/` snapshot against generated test-app artifacts and require `npm run setup-test-app` to have run first; `__tests__/{utils,workflows}/` is self-contained. Workflow tests may invoke focused Python behavior suites and therefore require Python 3. |
...
| A GitHub Actions workflow or its focused validation helper                                                                                    | `__tests__/workflows/`; keep it self-contained and invoke any Python helper suite explicitly                                                       |
